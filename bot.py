...
# -------------------- YouTube 解析（支援 cookies + 緩存） --------------------
yt_cache = {}  # 簡單緩存: query -> (url, title)
...
def get_audio_url(query):
    ...
    cookies_file = os.path.join(os.getcwd(), "cookies.txt")  # cookies 檔案
    ydl_opts = {
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            yt_cache[query] = (url, title)  # 存入緩存
            return url, title
    except yt_dlp.utils.DownloadError as e:
        raise Exception(f"無法取得影片: {e}")
